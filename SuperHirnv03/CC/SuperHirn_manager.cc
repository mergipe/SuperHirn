...
//  Created new functions and modified the indentation
// 
//
// **********************************************************************//
// CLASS Spec Manager:
...
////////////////////////////////////////////////////////////////////////////
// returns the basename of a file without its extension, given the filepath
string SuperHirn_manager::get_file_basename(string filepath) {
    size_t last_dot_index = filepath.find_last_of(".");
    string filepath_no_ext = filepath.substr(0, last_dot_index);
    char* filename_ch = new char[filepath_no_ext.length() + 1];
    strcpy(filename_ch, filepath_no_ext.c_str());

    return basename(filename_ch);
}

////////////////////////////////////////////////
// pop the reference run based on its filename
LC_MS* SuperHirn_manager::pop_reference_run(vector<LC_MS>* runs,
                                            string filename) {
    if (!runs->empty()) {
        LC_MS* ref;

        vector<LC_MS>::iterator p = runs->begin();
        while (p != runs->end()) {
            if (p->get_spec_name() == filename) {
                ref = new LC_MS(&(*p));
                runs->erase(p);
                return ref;
            }
...
////////////////////////////////////////////////////////////////////////
// write aligned runs in XML files
void SuperHirn_manager::write_aligned_runs(vector<LC_MS>* runs, LC_MS* ref) {
    string out_dir = SOURCE_DIR;
    ...
    tmp += out_dir + "LC_MS_RUNS_ALIGNED/";
    out_dir = tmp;
    mkdir(out_dir.c_str(), 0777);

    LCMSDataImporter* IMP = new LCMSDataImporter();

    if (runs != NULL && !runs->empty()) {
        vector<LC_MS>::iterator p = runs->begin();
        while (p != runs->end()) {
            IMP->write_out_LC_MS_run(&(*p), out_dir);
            p++;
        }

        IMP->write_out_LC_MS_run(ref, out_dir);
        printf("\n\t\tAligned LC-MS runs have been written to '%s'\n",
                out_dir.c_str());
    }
...
////////////////////////////////////////////////
// alignment of LC-MS runs with a reference run
void SuperHirn_manager::start_alignment_with_reference(vector<LC_MS>* runs,
                                                       LC_MS* ref) {
    printf("\tREFERENCE RUN: %s\n\n", ref->get_spec_name().c_str());
    multi_merger* MUL_ALL = new multi_merger(this,
            &SuperHirn_manager::get_LC_MS_by_ID,
            &SuperHirn_manager::delete_LC_MS_by_ID);

    vector<LC_MS>::iterator p = runs->begin();
    while (p != runs->end()) {
        printf("\t\t-- Alignment of LC-MS pair [%d, %d]\n",
                ref->get_spectrum_ID(), p->get_spectrum_ID());
        MUL_ALL->align_B_to_A(ref->get_spectrum_ID(), p->get_spectrum_ID());
        p++;
    }

    write_aligned_runs(runs, ref);
...
////////////////////////////////////////////////
// alignment of LC-MS runs with a reference run
void SuperHirn_manager::start_alignment_with_reference(string filename) {
    filename = get_file_basename(filename);
    parse_INPUT_DATA();

    if (get_ALIGNMENT_MAP_VECTOR() != NULL) {
        vector<LC_MS>* runs = get_ALIGNMENT_MAP_VECTOR();
        LC_MS* ref = pop_reference_run(runs, filename);

        if (ref != NULL) {
            start_alignment_with_reference(runs, ref);
        }
        else {
        ...
        printf("\t*** THERE IS NOTHING TO ALIGN! ***\n\n");
    }
}

////////////////////////////////////////////////
// alignment of LC-MS runs with a reference run
// however, here the first run is choosen to be the reference
void SuperHirn_manager::start_alignment_with_reference() {
    parse_INPUT_DATA();
