...
int check_run_modus(string run_status) {
    ...
    // intensity normalizataion of the master map
    ...
    if ((run_status == "-ALIGN_WITH_REFERENCE") || (run_status == "-AR")) {
        my_RUN.LastArgumentMode = 14;
        my_RUN.ArgumentModeON = true;
        return 14;
    }
...
void print_help() { 
    ...
    printf("\t\tC.\t-CREATE_MASTER(CM) (<PepXML PATH>):\t\tcreate a MasterMap by multiple LC-MS alignment\n");
    ...
    printf("\t\tI1.\t-ALIGN_WITH_REFERENCE(AR) <FILENAME>:\t\talign and save LC-MS runs in XML format, taking <FILENAME> as the reference run\n");
    ...
    while (PROCESS != my_RUN.SuperHirnProcessesIDs.end()) {
        ...
        if ((*PROCESS).second) {
            ...
            switch ((*PROCESS).first) {
                ...
                case 14:
                    //--------------------------------------------------------------------------
                    // align and save LC-MS runs in XML format, taking the argument as reference
                    printf("\n\n\t*** START LC-MS ALIGNMENT WITH REFERENCE... ***\n\n");

                    //--------------------------------------------------------------------------
                    // check here if any parameters are supplied specifying the reference file
                    F = my_RUN.SuperHirnProcessArguments.find((*PROCESS).first);
                    if (F != my_RUN.SuperHirnProcessArguments.end()) {
                        manager->start_alignment_with_reference((*F).second);
                    }
                    else {
                        manager->start_alignment_with_reference();
                    }
