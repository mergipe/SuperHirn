...
////////////////////////////////////////////////
// constructor for the object ms2_info:
ms2_info::ms2_info(string IN_SQ, float IN_PEP, int IN_CHRG, int IN_SCAN){
  PEP_PROB = IN_PEP;
  THEO_MZ = 0;
  MONO_MZ = 0;
  DELTA_CN = 0;
...
void ms2_info::set_THEO_MASS_from_SQ(){
  ...
  for(unsigned int POS = 0; POS < SQ.size(); POS++){
    ...
    if(P == MOD_LIST.end()){
      // get mass of the amino acid:
      TMP += getMolecularAminoAcidMass( SQ[POS] );
    }
    else{
      // add modified AA mass:
      TMP += (*P).second;
    }
  }
...
bool ms2_info::find_AC( string IN ){
  ...
  map<string,double>::iterator F = AC.find( IN );
  if( AC.end() == F ){
    return false;
  }
  else{
...
void ms2_info::set_AC(string IN, double prob){
  
  map<string,double>::iterator F = AC.find( IN);
  if( F == AC.end() ){
    AC.insert( make_pair( IN, prob) );
  }
}
...
bool ms2_info::search_AC_pattern(string IN){
  ...
  map<string,double>::iterator F = AC.begin();
  while( F != AC.end() ){
    if( F->first.find( IN ) != string::npos){
      return true;
    }
    F++;
  }
...
/////////////////////////////////////
// get the Molecular Mass of an Amino Acdi:
double ms2_info::getMolecularAminoAcidMass( char AA ){
