...
void LC_elution_peak::setSNIntensityThreshold(){
  ...
  while(P != get_signal_list_end()){
    ...
    // check MS/MS picked:
    if( peak->getPrecursorActivation( ) ){
      this->setChildScan( peak->getMSMSChildScanRT() );
    }
...
void LC_elution_peak::computeLCElutionPeakParameters( ){
  ...
  // set start et first scan in the LC_peak and
  // check if a peak has been picked by MS/MS
  while(P != get_signal_list_end()){
...
void LC_elution_peak::defineLCElutionPeakParametersFromMSPeak(){
  ...
  // check MS/MS picked:
  if( APEX->getPrecursorActivation( ) ){
    this->setChildScan( APEX->getMSMSChildScanRT() );
  }
  ...
  fSNIntensityThreshold = APEX->get_intensity() / APEX->getSignalToNoise();
