...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		mBottomNav = [[BottomNavigationViewController alloc] initWithNibName:@"BottomNavigationView" bundle:nil];
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
	self.navigationController.navigationBar.hidden = YES;
	[mBottomNav.view removeFromSuperview];
...
- (void)setupBottomNavigationView {
	...
	CGRect frame = mBottomNav.view.frame;
	frame.size.width = self.view.frame.size.width;
	frame.size.height = 150;
	frame.origin.y = self.view.frame.size.height;
	mBottomNav.view.frame = frame;
	[self.view addSubview:mBottomNav.view];
	mBottomNav.view.hidden = YES;
}
...
- (void)loadVerticalContentPaths {
	...
	for(int i = 1; i < spines.count(); i++) {
		...
		NSString *relBasePath = [NSString stringWithUTF8String:reader->rootRelativeFolderPath().toUtf8().data()];
		NSString *rPath = [relBasePath stringByAppendingPathComponent:relPath];
		NSString *contentFilePath = [basePath stringByAppendingPathComponent:rPath];
		//NSLog(@"Addin content path = %@", contentFilePath);
		[verticalContentPathList addObject:contentFilePath];
	}
}
...
- (void)loadHorizontalContentPaths {
	...
	//Get the navPoint from id
	NCXNavPoint *navPoint = NULL;
	QList<NCXNavPoint *> &navPoints = [navModel topLevelNavPointList];
	for(int i = 0; i < navPoints.count(); i++) {
		navPoint = navPoints[i];
		//Now create the list of horizontal content path
		QList<NCXNavPoint *> childrenNavPoints = navPoint->childrenNavPoints();
		QListIterator<NCXNavPoint *> i(childrenNavPoints);
		NSMutableArray *subArray = [[NSMutableArray alloc] init];
		while(i.hasNext()) {
			NCXNavPoint *cNavPoint = i.next();
			int playOrder = cNavPoint->playOrder();
			if(playOrder > 0) {
				OPFItem *item = ocfMap.value(cNavPoint->navPointID());
				if(item == NULL)
					continue;
				
				NSString *relPath = [NSString stringWithUTF8String:item->href().toUtf8().data()];
				NSString *hContentFilePath = [basePath stringByAppendingPathComponent:relPath];
				//Now figure out where this should go in the list
				if(playOrder > [horizontalContentPathList count]) {
					[subArray insertObject:hContentFilePath atIndex:playOrder-1];
				}
				else {
					[subArray addObject:hContentFilePath];
				}
			}
		}
		[horizontalContentPathList addObject:subArray];
		[subArray release];
	}
}
...
- (void)goUpToNextArticleCover {
	...
	//Make sure the navigation bar is shown
	self.navigationController.navigationBar.hidden = NO;
	if(navModel.currentSpineIndex + pageCount < spines.count()) {
		[[NSNotificationCenter defaultCenter] postNotificationName:@"ShowNextArticleCover" object:self];
	}
}
...
- (void)loadArticleWithBottomNavIndex:(NSNotification *)notification {
	...
	NavigationModel *navModel = [NavigationModel sharedModel];
	QList<NCXNavPoint *> navPoints = [navModel topLevelNavPointList];
	NCXNavPoint *navPoint = navPoints[index];
	NSString *articleID = [NSString stringWithUTF8String:navPoint->navPointID().toUtf8().data()];
	[self articleSelectedWithID:articleID];
	[self toggleFullScreenMode];
	currentArticleIndex = index;
}

- (void)toggleNavDisplay {
	mBottomNav.view.hidden = self.navigationController.navigationBar.hidden;
}

- (void)toggleFullScreenMode {
	
	if(self.navigationController.navigationBar.hidden) {
		mBottomNav.view.hidden = NO;
	}
...
- (IBAction)showTOC:(id)sender {
	...
	if (articleSelector == nil) {
		...
		[articleSelector setParentNavPoint:NULL];
...
- (void)articleSelectedWithID:(NSString *)articleId {
	NavigationModel *navModel = [NavigationModel sharedModel];
	//The article id we have is of the cover, we need that of the first
	//page of the content. Therefore, get the spine list and 
	//get the next the id of the first page of the content
	EPubReader *reader = [navModel epubReader];
	const QList<QString> spines = reader->spineList();
	QString aId([articleId UTF8String]);
	int index = spines.indexOf(aId);
	
	if(index < 0) {
		//We have to show error
		return;
	}
	[mScrollView setContentOffset:CGPointMake((index-1) * mScrollView.frame.size.width, 0.0) animated:YES];
	[self.articleSelectorPopover dismissPopoverAnimated:YES];
}
