...
- (void)buildArticlePathListWithNavPoint:(NCXNavPoint *)navPoint {
	...
	const EPubReader *reader = [navModel epubReader];
	QMap<QString, OPFItem *> ocfMap = reader->manifestMap();
	
	OPFItem *item = ocfMap.value(navPoint->navPointID());
	if(item == NULL) {
		return;
	}
	
	//Check if the navPoint has children navPoints or not
	QList<NCXNavPoint *>navPoints = navPoint->childrenNavPoints();
	for(int i = 0; i < navPoints.count(); i++) {
		NCXNavPoint *cNavPoint = navPoints[i];
		[self buildArticlePathListWithNavPoint:cNavPoint];
