...
//#import "iCompetenceEnvironment.h"
...
- (void)applyNewIndex:(NSInteger)newIndex toPageController:(OverviewPageViewController *)pController;
- (void)showPageAtIndex:(int)index inController:(OverviewPageViewController *)pController;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		libModel = [RecordModel sharedModel];
		[libModel loadOverviewRecords];
    }
    return self;
}
...
- (void)viewDidLoad {
	...
	int pageCount = [libModel.RecordList count]/9;
	if(pageCount == 0)
		pageCount = 1;
	...
	currentPage = [[OverviewPageViewController alloc] initWithNibName:@"OverviewPageView_iPad" bundle:nil];
	currentPage.view.frame = CGRectMake(0.0, 0.0, mScrollView.frame.size.width, mScrollView.frame.size.height);
	
	nextPage = [[OverviewPageViewController alloc] initWithNibName:@"OverviewPageView_iPad" bundle:nil];
...
- (NSUInteger)libPageCount {
	int pageCount = [libModel.RecordList count]/9;
	int m = [libModel.RecordList count] % 9;
	if(m > 0) {
		pageCount++;
	}
	return pageCount;
...
- (void)showPageAtIndex:(int)index inController:(OverviewPageViewController *)pController {
	...
	[pController loadOverviewForPageAtIndex:index];
}
...
- (void)loadMagazineAtIndex:(NSNotification *)notification {
	...
	NSString *fPath = [libModel.RecordList objectAtIndex:index];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)newScrollView {
    ...
    if (currentPage.pageIndex != nearestNumber) {
        OverviewPageViewController *swapPage = currentPage;
