...
@implementation OverviewPageViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
		...
		OverviewItems = [[NSMutableArray alloc] init];
    }
    return self;
}
...
- (void)loadOverviewForPageAtIndex:(int)index {
	...
	RecordModel *libModel = [RecordModel sharedModel];
	
	int sIndex = index * 9;
	int eIndex = sIndex + 9;
	if(eIndex > [libModel.RecordList count]) {
		eIndex = [libModel.RecordList count] - sIndex;
	}
	int libItemIndex = 0;
	int viewX = 50.0;
	int viewY = 50.0;
	...
	for(int i = sIndex; i < eIndex; i++) {
		...
		//First try to use the existing Overview items
		if([OverviewItems count] > libItemIndex) {
			itemView = [OverviewItems objectAtIndex:libItemIndex];
		}
		else {
			//Create a new one and find its location/frame
			itemView = [[OverviewItemView alloc] initWithFrame:CGRectMake(viewX, viewY, 175.0, 250.0)];
			itemView.delegate = self;
			[self.view addSubview:itemView];
			[OverviewItems addObject:itemView];
			[itemView release];
		}

		itemView.thumbnailView.image = [UIImage imageWithRecordsOfFile:[libModel.RecordList objectAtIndex:i]];
	...
	for(int j = libItemIndex; j < [OverviewItems count]; j) {
		OverviewItemView *itemView = [OverviewItems objectAtIndex:j];
		[itemView removeFromSuperview];
		[OverviewItems removeObjectAtIndex:j];
	}
}

#pragma mark BottomNavigationItemView delegate methods
- (void)tappedOnItem:(UIView *)view {
	NSLog(@"Tapped on Overview item");
	int index = [OverviewItems indexOfObject:view];
	if(index >= 0) {
