...
#import "GalleryNavigationViewController.h"


@implementation GalleryNavigationViewController
...
- (void)addNavItem:(NSString*) iTitle
{
	...
	GalleryNavigationItemView *v = [[GalleryNavigationItemView alloc] initWithFrame:CGRectMake(x, 0.0, ItemSpacing, ScrollViewHeight)];
	v.delegate = self;
...
- (void)addNavItemX:(NSString*) iTitle :(int)i
{
	...
	int x = ItemSpacing * count;
	GalleryNavigationItemView *v = [[GalleryNavigationItemView alloc] initWithFrameX:CGRectMake(x, 0.0, ItemSpacing, ScrollViewHeight): i];
	v.delegate = self;
	v.titleLabel.text = iTitle;
	//v.iconView.image = [UIImage imageWithContentsOfFile:iconPath];	
