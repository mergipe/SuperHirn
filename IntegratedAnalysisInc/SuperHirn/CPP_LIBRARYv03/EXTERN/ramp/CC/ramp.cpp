...
#define LOWMZ_UNINIT 1.111E6
   ...
   if (pFI->mzML) { // use pwiz lib to read mzML
	   MZML_TRYBLOCK;
	   pFI->mzML->getScanHeader(lScanIndex, *scanHeader);
	   MZML_CATCHBLOCK;
	   return;
   }
#endif
...
char *rampConstructInputPath(char *inbuf,int inbuflen,const char *dir_in,const char *basename_in) {
   char *result = NULL;
   int i;
   const char *basename = basename_in;
   char *dir = strdup(dir_in?dir_in:"");
   char *tmpbuf = (char *)malloc(strlen(dir) + strlen(basename) + 20);
   char *append;
   if (*dir)
   {  // make sure this is a directory, and not a directory+filename
      struct stat buf;
	  if ((!stat(dir,&buf)) && !S_ISDIR(buf.st_mode)) { // exists but is not a directory
      ...
      ramp_fileoffset_t lScanIndex)
{
   ...
   if (pFI->mzML) { // use pwiz lib to read mzML
	   ...
	   pFI->mzML->getScanPeaks(lScanIndex, vec);
